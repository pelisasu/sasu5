...
class Settings:
    ...
    min_tp_pips: float = float(os.getenv("MIN_TP_PIPS", "500"))
    min_rr: float = float(os.getenv("MIN_RR", "2.5"))
    strategy_score_threshold: float = float(os.getenv("STRATEGY_SCORE_THRESHOLD", "60"))
    ai_confidence_threshold: float = float(os.getenv("AI_CONFIDENCE_THRESHOLD", "70"))
