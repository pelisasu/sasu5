...
class InstrumentResolver:
    ...
    def resolve_xauusd(self, preferred="frxXAUUSD"):
        ...
        # 3. Lamun tetep teu manggihan, tampilkeun sampel simbol ti server
        all_syms = [s.get("symbol") for s in symbols[:20]]
        raise RuntimeError(f"Symbol {preferred} not found. Sample available symbols: {', '.join(map(str, all_syms))}")
